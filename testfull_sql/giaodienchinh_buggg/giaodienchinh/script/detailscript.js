...
function checkls() {
    ...
    listdata = localStorage.getItem("data") ? JSON.parse(localStorage.getItem("data")) : []
...
function MuaNgay() {
    var url = new URL(window.location.href)
    let productId = url.searchParams.get("productId");
    alert("proID là: " + productId)
    if (listdata.length != 0) alert("uID là: " + listdata[0].id)
    else alert("chưa có uid")
    alert("số lượng là: " + soluongg.value)
}